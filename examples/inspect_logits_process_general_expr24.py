...
def main():
    parser = argparse.ArgumentParser(
        description="Inspect GrammarIncrementalLogitsProcessorGeneral acceptances (expr24)."
    )
    parser.add_argument(
        "--grammar-path",
        default=str(Path(__file__).resolve().parent / "grammars" / "24_points.ebnf"),
        help="EBNF grammar file path",
    )
    parser.add_argument(
        "--vocab",
        default=None,
        help="Comma or space-separated vocab tokens (defaults to digits+ops+<eos>)",
    )
    parser.add_argument(
        "--nice-tokens",
        default=None,
